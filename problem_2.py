...
def weighted_row_sum_kernel(
    ...
    row_start_ptr = X_ptr + row_idx * N_COLS # this has to be multiply by N_COLS because we are moving row by row, each row has N_COLS elements
    # remeber about how data is stored in memory in C
    
    # 3. Create a pointer for the output vector Y.
    output_ptr = Y_ptr + row_idx # no need to * N_COLS because Y is 1D, each row has 1 element
